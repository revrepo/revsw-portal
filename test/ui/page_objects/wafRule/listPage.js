...
var CustomerWAFRulesTable = require('./table-customer/table');
...
var WAFList = {
  ...
  locators: {
    ...
    tabs: {
      customer: {
        css: '.waf-rules__tabs .nav.nav-tabs>li:nth-child(1) a'
      },
      builtin: {
        css: '.waf-rules__tabs .nav.nav-tabs>li:nth-child(2) a'
      }
    }
  },
  ...
  // those components.
  // TODO: add table PO
  tableCustomerRules: CustomerWAFRulesTable,
  ...
  getLearnFromTheLogsAndAddAutoGeneratedRuleBtn: function () {
    ...
    /**
   * ### WAFList.getCustomerWAFRuleTab()
   *
   * Returns the reference to the tab `Customer Rules`
   * (Selenium WebDriver Element) from the WAF Rule page from the Portal app.
   *
   * @returns {Object} Selenium WebDriver Element
   */
  getCustomerWAFRuleTab: function(){
    return element(by.css(this.locators.tabs.customer.css));
  },
   ...
   */
  searchAndGetFirstRowInCustomerTable: function (criteria) {
    this.searcher.clearSearchCriteria();
    this.searcher.setSearchCriteria(criteria);
    this.getCustomerWAFRuleTab().click();
    return this.tableCustomerRules
      .getFirstRow();
  },
  ...
  /**
   * ### WAFList.searchAndClickEditCustomerRule()
   *
   * Filters the WAF Rule table by the given criteria and triggers a click
   * on the `Edit` button of the first result of the table `Customer Rules`.
   *
   * @param {String} criteria, to filter
   *
   * @returns {Object} Promise
   */
  searchAndClickEditCustomerRule: function (criteria) {
    return this
      .searchAndGetFirstRowInCustomerTable(criteria)
      .clickEdit();
  }
